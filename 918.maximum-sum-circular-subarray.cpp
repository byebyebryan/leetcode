 ...
 public:
  int maxSubarraySumCircular(vector<int>& nums) {
    ...
    for (int i = 1; i < nums.size(); i++) {
      ...
      exclMax = max(inclMax, exclMax);
      exclMin = min(inclMin, exclMin);

      inclMax = max(inclMax + n, n);
      inclMin = min(inclMin + n, n);

      sum += n;
    }

    int maxSum = max(exclMax, inclMax);
    int minSum = min(exclMin, inclMin);
